<!-- Banner Premium Gerado Localmente -->
<p align="center">
  <img src="https://raw.githubusercontent.com/leon-bs17/leon-bs17/main/assets/banner.svg" alt="Leonardo Barroso Banner" width="100%" />
</p>

<!-- Badges Sociais Alinhadas e Minimalistas -->
<p align="center">
  <a href="https://www.linkedin.com/in/leonardo-barroso-0409023a9" target="_blank">
    <img src="https://img.shields.io/badge/-LinkedIn-%230077B5?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn" />
  </a>
  &nbsp;&nbsp;
  <a href="mailto:leonardosousa1725@gmail.com">
    <img src="https://img.shields.io/badge/-Gmail-%23D14836?style=for-the-badge&logo=gmail&logoColor=white" alt="Gmail" />
  </a>
  &nbsp;&nbsp;
  <img src="https://komarev.com/ghpvc/?username=leon-bs17&style=for-the-badge&color=10B981&label=VISITAS" alt="Contador de Visitas" />
</p>

<br/>

<!-- Layout de Colunas Premium (Sem indentação para evitar blocos de código automáticos no Markdown) -->
<table border="0" cellpadding="0" cellspacing="0" width="100%">
<tr>
<td valign="top" width="55%" style="padding-right: 15px;">

### 🚀 Sobre Mim
Olá! Sou o **Leonardo**, desenvolvedor focado no ecossistema **Back-end**. Gosto de projetar APIs robustas, arquiteturas limpas e soluções eficientes. Minha meta é escrever código que seja fácil de manter e escalar.

Atualmente trabalhando com **Python (FastAPI)** e **TypeScript (Next.js)** para criar soluções web completas e inteligentes.

<br/>

### 💼 Projetos em Destaque

* **[💰 FinanceFlow](https://github.com/leon-bs17/finace-flow)**  
  SaaS de finanças pessoais com Inteligência Artificial. Importação automática de extratos bancários, categorização inteligente de gastos, controle de assinaturas e assistente financeiro via LLM.  
  `FastAPI` • `Next.js 14` • `SQLite` • `SQLAlchemy`

* **[🖥️ PC Health Monitor](https://github.com/leon-bs17/pc-health-monitor)**  
  Interface desktop em Python para monitorar estatísticas do computador em tempo real, integrada com IA local para analisar a saúde do hardware.  
  `Python` • `Flet` • `Psutil` • `Groq AI`

</td>
<td valign="top" width="45%">

### 🛠️ Minhas Tecnologias
Abaixo estão as ferramentas e linguagens que utilizo no meu dia a dia de desenvolvimento:

<p align="left">
  <a href="https://skillicons.dev">
    <img src="https://skillicons.dev/icons?i=py,ts,js,fastapi,postgres,sqlite,next,react,tailwind,git,docker,vscode,github" />
  </a>
</p>

<br/>

### 📊 Estatísticas

<!-- Usando o servidor estendido github-stats-extended que é muito mais estável e possui cache robusto -->
<a href="https://github.com/stats-organization/github-stats-extended">
  <img width="100%" src="https://github-stats-extended.vercel.app/api?username=leon-bs17&show_icons=true&theme=dark&hide_border=true&bg_color=0d1117&title_color=10B981&icon_color=06B6D4&text_color=8b949e&ring_color=10B981" alt="GitHub Stats" />
</a>

<br/><br/>

<!-- Usando o demolab oficial (que não sofre com bloqueio de IP do GitHub) e aplicando o cache_bust (v=3) -->
<a href="https://github.com/DenverCoder1/github-readme-streak-stats">
  <img width="100%" src="https://streak-stats.demolab.com/?user=leon-bs17&theme=dark&hide_border=true&background=0d1117&ring=10b981&fire=10b981&currStreakLabel=10b981&sideLabels=8b949e&dates=8b949e&stroke=10b981&v=3" alt="GitHub Streak" />
</a>

</td>
</tr>
</table>

<br/>
<hr/>
<p align="center">
  <sub>📍 Fortaleza, CE &nbsp;•&nbsp; ✉️ <a href="mailto:leonardosousa1725@gmail.com">leonardosousa1725@gmail.com</a></sub>
</p>
